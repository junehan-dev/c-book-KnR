...
- 함수 포인터 정의, 함수 포인터화 예시
   ...
   - Argument로 전달되는 함수는 함수포인터임을 호출 함수의 parameter에 명시되어 있으므로, ``&strcmp``\의 처리는 필요하지 않다. 함수는 그 존재 자체로 값이 아니라, 포인터.

CH06-Structures
===============

- 구조체는, 하나 혹은 복수의 변수들의 collection.

   - 아마도 그들은 다른 타입일 가능성이 높으며,
   - 편리한 취급을 위해 하나의 그룹화 하는 것.

      - *특정언어들은 Structures를 *records*\라고 부른다.*
- ANSI표준에서 주요한 변화 중 하나는 structure assignment를 정의하는 방식이다.

   - structures는 복사와 할당이 가능하다.
   - 함수에 전달이 가능하다.
   - 함수에 의해 값으로 돌아올 수 있다.
   - 자동 structures와 arrays는 initialized 될 수 있다.
...
- 구조체 형 정의와 변수 선언:
   ``struct { ... } x, y, z;``
- *변수명의 리스트가 따르지 않는 구조체 정의는 어떤 공간도 차지하지 않는다.*

   - (아주 작은 공간을 사용해 구조체의 모양 혹은 템플릿 정보만 가진다.)
- ``point``\타입 구조체 변수 선언:
   ``struct point pt;``
- ``point``\타입 구조체 변수의 값 할당:
   ``pt = { 320, 200 };``

6.3 Array of Structures
-----------------------

- 구조체 배열은 아래와 같은 형태로 선언이 가능하다.
   .. code-block:: c
...
- 배열의 길이가 정해진 경우와 다르게, 컴파일러가 추론하게 만드는 경우,

   - ``sizeof(points) / sizeof(struct point);`` 를 통해 길이를 가져올 수 있다.
